<script>
    ...
    async function getSuicide() {
        ...
        if (res.ok) {
            ...
            okPrint = "";
            errorPrint =
                "No se han encontrado registros con dichos parámetros.";
            infoPrint = "";
        } else {
            okPrint = "";
    ...
    async function initialSuicide() {
        ...
        const data = await fetch(BASE_API_PATH + "/loadInitialData").then(
            function (data) {
                if (data.ok) {
                    console.log("OK");
                    ...
                    errorPrint =
                        "Limpie la base de datos antes de cargarla de nuevo.";
                    okPrint = "";
                    infoPrint = "";
                } else {
                    console.log("ERROR");
    ...
    async function insertSuicide() {
        ...
        const res = await fetch(BASE_API_PATH, {
            ...
            if (res.status == 201) {
                ...
                errorPrint =
                    "Un registro con las mismas claves primarias ya existe en la base de datos.";
                okPrint = "";
                infoPrint = "";
            } else if (res.status == 500) {
                errorPrint =
                    "Error del servidor al intentar guardar un registro.";
                okPrint = "";
                infoPrint = "";
            }
        });
    ...
    async function deleteSuicide(provincia, anyo) {
        ...
        const res = await fetch(BASE_API_PATH + "/" + provincia + "/" + anyo, {
            method: "DELETE",
        }).then((res) => {
            if (res.status == 200) {
                suicides.push(newSuicide);
                ...
                errorPrint =
                    "No hay ningún registro a eliminar con dichas claves.";
                okPrint = "";
                infoPrint = "";
            } else if (res.status == 500) {
                errorPrint =
                    "Error interno del servidor al eliminar el recurso.";
                okPrint = "";
                infoPrint = "";
            }
        });
    ...
    function changePage(pageX, offsetX) {
        console.log(
            "Changing page to " + pageX + " with " + offsetX + " offset."
        );
    ...
    const creaParametros = () => {
        if (estructuraBusqueda.province.length != 0) {
            console.log("TIENE0");
            paramsBusqueda =
                paramsBusqueda + "&province=" + estructuraBusqueda.province;
        }
        if (estructuraBusqueda.year.length != 0) {
            console.log("TIENE");
            paramsBusqueda =
                paramsBusqueda + "&year=" + estructuraBusqueda.year;
        }
        if (estructuraBusqueda.suic_man.length != 0) {
            paramsBusqueda =
                paramsBusqueda + "&suic_man=" + estructuraBusqueda.suic_man;
        }
        if (estructuraBusqueda.suic_woman.length != 0) {
            paramsBusqueda =
                paramsBusqueda + "&suic_woman=" + estructuraBusqueda.suic_woman;
        }
        if (estructuraBusqueda.suic_total.length != 0) {
            paramsBusqueda =
                paramsBusqueda + "&suic_total=" + estructuraBusqueda.suic_total;
        }
        if (estructuraBusqueda.suic_rate_mw.length != 0) {
            paramsBusqueda =
                paramsBusqueda +
                "&suic_rate_mw=" +
                estructuraBusqueda.suic_rate_mw;
        }
...
<main>
    ...
    <Table bordered>
        ...
        <tbody>
            <tr>
                ...
                <Button
                    color="primary"
                    on:click={creaParametros}
                    style="margin:10px;">Buscar</Button
                >
            </tr>
        </tbody>
    </Table>
    ...
    <tr>
        <td
            ><Button
                on:click={initialSuicide}
                color="primary"
                style="margin:5px">Generar Lista</Button
            ></td
        >
        ...
        <td
            ><Button style="background-color:orange"
                ><a
                    href="http://sos2021-27.herokuapp.com/#/graphics/suicide-records"
                    >Gráfico</a
                ></Button
            ></td
        >
    ...
    <Table bordered>
        ...
        <tbody>
            <tr>
                ...
                <td
                    ><Button
                        style="background-color:#32D524;"
                        on:click={insertSuicide}>Insertar Registro</Button
                    ></td
                >
            </tr>
    ...
    <div>
        <td align="left">
            <Button
                outline
                color="info"
                href="https://sos2021-27.herokuapp.com/#/info"
            >
                Página principal
            </Button>
        </td>
        <td style="float: right;">
            <Pagination>
                <PaginationItem class={page === 1 ? "disabled" : ""}>
                    <PaginationLink
                        previous
                        href="#/suicide-records"
                        on:click={() => changePage(page - 1, offset - 10)}
                    />
                </PaginationItem>
                {#each range(lastPage, 1) as pages}
                    <PaginationItem class={page === pages ? "active" : ""}>
                        <PaginationLink
                            previous
                            href="#/suicide-records"
                            on:click={() => changePage(pages, (pages - 1) * 10)}
